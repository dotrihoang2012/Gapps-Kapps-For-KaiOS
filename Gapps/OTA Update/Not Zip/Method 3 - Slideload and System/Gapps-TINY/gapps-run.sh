...
/tmp/busybox cat /tmp/webapps-system.json /tmp/gapps-distribution/decl-system.patch > $WEBAPPS_SYSTEM
...
if /tmp/busybox [ -f $WEBAPPS_DATA ]; then
    ...
    /tmp/busybox cat /tmp/webapps-data.json /tmp/gapps-distribution/decl-data.patch > $WEBAPPS_DATA
else
    echo -n "{" > /tmp/webapps-data.json
    /tmp/busybox sed 's/^,//' /tmp/gapps-distribution/decl-data.patch > /tmp/decl-nocomma.patch
